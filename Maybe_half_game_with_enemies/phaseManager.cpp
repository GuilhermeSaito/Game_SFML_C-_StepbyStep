...
{
	float posxp = Player::getInstance()->getPlayer1Position().x;
	float posyp = Player::getInstance()->getPlayer1Position().y;
...
void PhaseManager::updateEnemies2()
{
	...
	else if (this->enemyQuantity == 2)
	{
		if (posxp >= 2000)
		{
			this->bosses.push_back(new Boss(4400, 400, 100));
			this->enemyQuantity++;
		}
	}
