...
void PhaseManager::render(int i, sf::RenderWindow& window)
{
	...
	if (player1->getPosition().y > 608 || player1->playerDie())
	{
		// Go to "Wasted screen" (not implemented yet)
		gameState = 2;
		return;
	}
	// Se der algum erro com memória, no inimigo, pode ser aqui
	enemy1Die();
	enemy2Die();
	if (boss.bossDie())
	{
		// Go to Ranking Screen (not implemented yet)
		player1->gainPoints(boss.getBossPoint());
		gameState = 2;
		return;
	}
...
void PhaseManager::enemy1Die()
{
	// SE DER ALGUM ERRO NO INIMIGO, PROVAVELMENTE É AQUI.
	for (Enemy1* e : enemies1)
	{
		auto it = enemies1.begin();
		if (e->getPosition().y > 624 || e->enemyDie())
		{
			player1->gainPoints(e->getEnemy1Point());
			enemies1.erase(it);
...
void PhaseManager::enemy2Die()
{
	...
	for (Enemy2* e : enemies2)
	{
		...
		if (e->getPosition().y > 624 || e->enemyDie())
		{
			player1->gainPoints(e->getEnemy2Point());
			enemies2.erase(it);
